...
import { EducationEditorErrors, PublicationItem } from './educationEditor.slice';
...
type EducationHeadEditorProps = {
  ...
  category: string;
  loading?: boolean;
  onChange: (publication: Partial<PublicationItem>) => void;
  attachment?: string;
...
const EducationHeadEditor = ({
  ...
  const handleAttachmentChange = useCallback(
    (a: string) => onChange({ attachment: a }),
    [onChange]
  );
  ...
  return (
    <Container>
      ...
      <Section>
        ...
        <LimitsCounter current={title.length} max={MAX_TITLE_LENGTH} error={!!errors?.title.empty}>
          <TitleTextField
            lighten
            type="text"
            value={title}
