...
import usePreloadImages from '../task-management/survey/survey-editor/common/usePreloadImages';
  ...
  const uploadFile = useCallback(async () => {
    if (outgoingFile instanceof File) {
      const res = await upload({
        path: getUploadObjectPath(outgoingFile),
        file: outgoingFile,
        generateDownloadUrl: true,
        downloadUrlExpiresAfterMs: Duration.fromObject({ year: 1 }).toMillis(),
      });
      if (!res.err && res.url) {
