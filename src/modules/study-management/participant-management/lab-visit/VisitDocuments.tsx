...
import { MAX_ALL_DOCUMENTS_SIZE, MAX_DOCUMENT_SIZE } from './labVisit.slice';
        ...
        {(() => {
          ...
          return (
            <FileItemAction onClick={handleRemove}>
              <RemoveIconStyled />
              <FileItemSize>{humanFileSize(item.sizeBytes)}</FileItemSize>
            </FileItemAction>
          );
        })()}
      </FileItemPreview>
...
type VisitDocumentsProps = {
  ...
  setAfterSent: (params: { cb: ((filePath: string) => Promise<void>) | undefined }) => void;
};
...
export const VisitDocuments = ({
        ...
        const documentsOperationsQueue: Array<() => Promise<boolean>> = [
          ...documentsToRemove.map((d) => async () => {
            try {
              ...
              docs = docs
                .map((f) => (f.id === d.id ? undefined : f))
                .filter(Boolean) as DocumentEntry[];
              return true;
            } catch {
              docs = docs.map((f) => (f.id === d.id ? { ...f, hasError: true } : f));
              return false;
            }
          }),
          ...documentsToUpload.map((d) => async () => {
            try {
              ...
              await uploadObject({
                studyId,
                name: getFilePath(filePath, d.name),
                blob: d.file,
              });
  ...
  const [rawFilesList, fetchFilesList] = useAsyncFn(async () => {
    if (!studyId || !filesPath || documents.length) {
      return;
    }

    const list = await listObjects({ studyId, path: filesPath });
    setDocuments(list.map((i) => ({ ...i, id: uuid() })));
  }, [filesPath, studyId, documents]);
