...
import { LabVisitItem } from './labVisit.slice';
...
const VisitNoteModal = ({ data, onRequestClose, onEdit, ...props }: VisitNoteModalProps) => {
  ...
  return (
    ...
    >
      <Header>
        <HeaderCell title="Participant ID" description={cachedData?.participantId ?? '—'} />
        <HeaderCell
          title="Visit Date"
          description={
            cachedData?.startTs ? dt.format(cachedData.startTs, 'ccc, LLL dd, yyyy') : '—'
          }
        />
        <HeaderCell
          title="Visit Time"
          description={cachedData?.startTs ? dt.format(cachedData.startTs, 'hh:mm a') : '—'}
        />
      </Header>
      <NoteBox>{cachedData?.notes}</NoteBox>
    </Modal>
  );
};
